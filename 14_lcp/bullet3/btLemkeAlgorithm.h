...
{
public:
	btLemkeAlgorithm(const btMatrixXu& M_, const btVectorXu& q_, const int& DEBUGLEVEL_ = 0) : BT_DEBUGLEVEL(DEBUGLEVEL_)
	{
		setSystem(M_, q_);
	}
   ...
   */
	btVectorXu solve(unsigned int maxloops = 0, bool useImprovedLexicoMimimum = false);
...
protected:
	int findLexicographicMinimum(const btMatrixXu& A, const int& pivotColIndex, const int& z0Row);
	int findLexicographicMinimum2(const btMatrixXu& A, const int& pivotColIndex, const int& z0Row);
